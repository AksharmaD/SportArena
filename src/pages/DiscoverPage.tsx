...
import { SPORT_OPTIONS, SKILL_LEVELS, SPORT_EMOJIS, type Profile } from '@/types';
...
export function DiscoverPage() {
  ...
  const fetchAthletes = useCallback(async () => {
    ...
    if (filterSport) {
      query = query.contains('sports', [filterSport]);
    }
    if (filterLocation) {
      query = query.ilike('city', `%${filterLocation}%`);
    }
    if (filterSkill) {
      query = query.eq('skill_level', filterSkill);
    }

    const { data } = await query.limit(20);
    setAthletes((data as Profile[]) || []);
    setLoading(false);
  }, [user, filterSport, filterLocation, filterSkill]);
  ...
  const hasFilters = filterSport || filterLocation || filterSkill;
    ...
    <div className="min-h-screen bg-ink-50/60 pt-16">
      <div className="container-px py-8">
        <div className="mx-auto max-w-5xl">
          <h1 className="font-display text-2xl font-extrabold text-ink-950">Discover Athletes</h1>
          <p className="mt-1 text-sm text-ink-500">
            Find people who play your sport, at your level, near you.
          </p>
          ...
          <div className="mt-6 rounded-2xl border border-ink-100 bg-white p-4 shadow-soft">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-ink-500">Location</label>
                <input
                  type="text"
          ...
          {loading ? (
            ...
            <div className="py-20 text-center">
              ...
              <p className="text-xs text-ink-400">Try adjusting your filters or check back later.</p>
            </div>
          ) : (
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {athletes.map((athlete) => {
                ...
                const isSelf = athlete.id === user?.id;

                // Good match: shares a sport with current user
                const sharedSports = profile?.sports?.filter((s) => athlete.sports?.includes(s)) || [];
                const isGoodMatch = sharedSports.length > 0;

                return (
                  <div
                    key={athlete.id}
                    className="group relative overflow-hidden rounded-2xl border border-ink-100 bg-white p-5 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-card"
                  >
                    {isGoodMatch && (
                      ...
                      <div>
                        ...
                        <p className="text-sm text-ink-500">
                          {athlete.sports?.length > 0
                            ? athlete.sports.slice(0, 2).map((s) => `${SPORT_EMOJIS[s] || ''} ${s}`).join(' · ')
                            : 'No sports listed'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-ink-400">
                      {athlete.skill_level && (
                        <span className="rounded-full bg-ink-50 px-2.5 py-0.5 text-xs font-semibold text-ink-600">
                          {athlete.skill_level}
                        </span>
                      )}
                      {athlete.city && (
                        <span className="flex items-center gap-1">
                          ...
                          {athlete.city}
                          {athlete.area ? `, ${athlete.area}` : ''}
                        </span>
                      )}
                    </div>

                    {athlete.bio && (
                      <p className="mt-3 line-clamp-2 text-sm text-ink-500">{athlete.bio}</p>
                    )}
